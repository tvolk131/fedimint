...
impl Gateway {
    ...
    async fn new(
        ...
        Ok(Self {
            ...
            federation_manager: Arc::new(FederationManager::new(client_builder)),
    ...
    pub async fn clone_state(&self) -> GatewayState {
    ...
    pub async fn handle_htlc_stream(&self, mut stream: RouteHtlcStream<'_>, handle: TaskHandle) {
        let GatewayState::Running { lightning_context } = self.clone_state().await else {
    ...
    pub async fn handle_get_info(&self) -> Result<GatewayInfo> {
        if let GatewayState::Running { lightning_context } = self.clone_state().await {
    ...
    pub async fn handle_get_federation_config(
        ...
        if let GatewayState::Running { .. } = self.clone_state().await {
    ...
    async fn handle_connect_federation(
        ...
        if let GatewayState::Running { lightning_context } = self.clone_state().await {
            ...
            *next_scid =
            ...
            self.federation_manager
                .client_builder
                .save_config(gw_client_cfg, dbtx)
                .await?;
    ...
    pub async fn handle_set_configuration_msg(
        ...
        let gw_state = self.clone_state().await;
    ...
    async fn load_clients(&self) {
        ...
        for config in configs.clone() {
            ...
            if let Ok(client) = Box::pin(Spanned::try_new(
                ...
                self.federation_manager
                    .client_builder
                    .build(config, Arc::new(self.clone())),
            ))
            .await
            {
        ...
        task_group.spawn_cancellable("register clients", async move {
            loop {
                ...
                if let Some(gateway_config) = gateway_config {
                    let gateway_state = gateway.clone_state().await;
