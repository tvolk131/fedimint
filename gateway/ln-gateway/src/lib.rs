...
impl Gateway {
    ...
    pub async fn handle_get_info(&self) -> Result<GatewayInfo> {
        let GatewayState::Running { lightning_context } = self.get_state().await else {
            return Ok(GatewayInfo {
                ...
                gateway_state: self.lightning_manager.get_state_string().await,
                network: None,
                block_height: None,
        ...
        let (clients_by_federation_id, federation_ids_by_scid) = {
            let federation_manager = self.federation_manager.read().await;
            (
                federation_manager.clone_client_map(),
                federation_manager.clone_scid_map(),
            )
        };

        let mut federations = Vec::new();
        for (federation_id, client) in clients_by_federation_id {
            federations.push(
                client
                    .borrow()
        ...
        Ok(GatewayInfo {
            federations,
            channels: Some(federation_ids_by_scid),
            version_hash: fedimint_build_code_version_env!().to_string(),
    ...
    pub async fn handle_get_federation_config(
        ...
        if let Some(federation_id) = federation_id {
            ...
            for (federation_id, client) in self.federation_manager.read().await.iter_clients() {
                federations.insert(
                    *federation_id,
                    client.borrow().with_sync(|client| client.get_config_json()),
                );
    ...
    async fn register_federations(
        ...
        let Ok(lightning_context) = self.lightning_manager.get_lightning_context().await else {
            return Ok(());
        };
    ...
    async fn load_clients(&self) {
        ...
        for config in configs.clone() {
            ...
            .await
            {
                federation_manager.add_client(scid, federation_id, client);
            } else {
    ...
    pub async fn leave_all_federations(&self) {
        ...
        let keypair = dbtx
            ...
            .await
            .expect("Gateway keypair does not exist");
        for (_, client) in self.federation_manager.read().await.clone_client_map() {
            client
                .value()
                .get_first_module::<GatewayClientModule>()
                .remove_from_federation(keypair)
...
impl Gateway {
    ...
    async fn send_payment_v2(
        ...
        self.federation_manager
            .read()
            .await
            .get_client(payload.federation_id)
